...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        r = requests.get(f'https://api.profiles.wprdc.org/domain/')
        taxonomy = r.json()
        for domain in taxonomy['results']:
            ...
            for subdomain in domain['subdomains']:
                ...
                for indicator in subdomain['indicators']:
                    ...
                    subdomain_indicator_obj, _ = SubdomainIndicator.objects.get_or_create(
                        subdomain=subdomain_obj,
                        indicator=indicator_obj,
                    )
                    subdomain_indicator_obj.order = i
                    subdomain_indicator_obj
                    i += 1
